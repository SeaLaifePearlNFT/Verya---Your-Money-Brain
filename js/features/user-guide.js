...
(function(){
  ...
  function maybeOpenWelcome(){
    var data = loadWelcome();
    // v760 — auto-show whenever the budget is fresh; the freshness gate below
    // stops it once the user actually starts budgeting. 'started'/'completed'
    // still suppress it (they've already engaged with onboarding).
    if (data.started || data.completed) return;
